...
if tool == "体验画像对比（双山峰）":
    ...
    if overlap_pct >= 75:
        st.success("整体很接近：你更多是在“微调口味”。可以聚焦差距最大的那一轴，提几个具体例子（更容易被理解）。")
    elif overlap_pct >= 45:
        st.warning("有明显差距：建议优先围绕差距最大维度，说清楚“现在在哪、我想要哪”，再配两三个具体例子。")
    else:
    ...
    with left:
        ...
        fig_bar.update_layout(
            barmode="group",
            height=360,
            margin=dict(l=20, r=10, t=30, b=70),
    ...
    with right:
        ...
        fig_slice.update_layout(
            ...
            title=f"2D 切片（固定 {fixed_name}≈{fixed_value:.1f}）",
            legend=dict(orientation="h", yanchor="top", y=-0.18, xanchor="left", x=0),
        )
